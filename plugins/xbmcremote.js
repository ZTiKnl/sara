...
function up() {
  return new Promise(resolve => {
    send_input(remote_inputs["Up"], function(err, resp) {
      if (err) {
        response.conlog('plugin/xbmcremote', err.message, 'error');
      }
      response.conlog('plugin/xbmcremote', resp, 'data');
      if (resp.result == 'OK') {
        result = ['Up'];
      } else {
...
function do_request(reqpayload, fn) {
  ...
  response.conlog('plugin/xbmcremote', 'Performing request: '+JSON.stringify(xbmc_options.body), 'data');
...
function send_input(input, fn) {
  ...
  do_request(payload, function(error, resp) {
    if (!error) {
      response.conlog('plugin/xbmcremote', 'Send input callback got ok, response: '+JSON.stringify(resp), 'data');
    }
    fn(error, resp);
  });
};
...
function send_input_action(inputaction, fn) {
  ...
  do_request(payload, function(error, resp) {
      if (!error) {
        response.conlog('plugin/xbmcremote', 'Send input action callback got ok, response: '+JSON.stringify(resp), 'data');
      }
      fn(error, resp);
    });
};
...
function get_player(fn) {
  ...
  do_request(payload, function(error, resp) {
    if (error) {
        response.conlog('plugin/xbmcremote', 'error getting player: '+JSON.stringify(resp), 'error');
    } else {
        response.conlog('plugin/xbmcremote', 'got player: '+JSON.stringify(resp), 'data');
    }
    fn(error, resp);
  });
};